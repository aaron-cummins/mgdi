...
import FormEstadoMotorInstalacion from 'pages/estadoMotorInstalacion/components/FormEstadoMotorInstalacion';
import  EstadoMotorInstalacion from 'pages/estadoMotorInstalacion/EstadoMotorInstalacion';
...
describe('Estado motor instalación', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Estado Motor Instalación')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Estado Motor Instalación" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Estado Motor Instalación");
    }) 
    ...
    it('Verifica que el mensaje de error desaparezca cuando se ingresan datos válidos en el campo Nombre del formulario', () => {
        ...
        fireEvent.change(InputNombreElemento , { target: { value: 'Estado Motor Instalación 1' } });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockEstadoMotorInstalacionContext = {
            EstadoMotorInstalacionActual: null,
            registrarEstadoMotorInstalacion: jest.fn(),
            actualizarEstadoMotorInstalacion: jest.fn(),
            obtenerEstadoMotorInstalacion: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <EstadoMotorInstalacionContext.Provider value={mockEstadoMotorInstalacionContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormEstadoMotorInstalacion closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Estado Motor Instalación 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockEstadoMotorInstalacionContext = {
            EstadoMotorInstalacionActual: null,
            registrarEstadoMotorInstalacion: jest.fn().mockResolvedValue({
                mensaje: "Estado Motor Instalacion creado con exito!",
                tipoAlerta: "success",
              }),
            actualizarEstadoMotorInstalacion: null,
            obtenerEstadoMotorInstalacion: jest.fn(),
          };
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Estado Motor Instalacion" } });
        ...
        await waitFor(() => {
            expect(mockEstadoMotorInstalacionContext.registrarEstadoMotorInstalacion).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Estado Motor Instalacion',
                activo: true
            });
        });

        const mensajeExitoso = screen.getByText("Estado Motor Instalacion creado con exito!");
