...
import FormMonitoreoMotor from 'pages/monitoreoMotor/components/FormMonitoreoMotor';
import MonitoreoMotor from 'pages/monitoreoMotor/MonitoreoMotor';
...
describe('Monitoreo Motor', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Monitoreo Motor')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Monitoreo Motor" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Nuevo Monitoreo Motor");
    }) 
    ...
    it('Verifica que se muestre el mensaje de error cuando los campos obligatorios se dejan en blanco al hacer clic en el botón guardar', () => {
        ...
        expect(screen.getByText(/Nombre de monitoreo motor requerido/i)).toBeInTheDocument();
      });
    ...
    it('Verifica que el mensaje de error desaparezca cuando se ingresan datos válidos en el campo Nombre del formulario', () => {
        ...
        fireEvent.change(InputNombreElemento , { target: { value: 'Tipo salida 1' } });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockMonitoreoMotorContext = {
            monitoreomotorActual: null,
            registrarMonitoreoMotor: jest.fn(),
            actualizarMonitoreoMotor: jest.fn(),
            obtenerMonitoreoMotor: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <MonitoreoMotorContext.Provider value={mockMonitoreoMotorContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormMonitoreoMotor closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Monitoreo Motor 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockMonitoreoMotorContext = {
            monitoreomotorActual: null,
            registrarMonitoreoMotor: jest.fn().mockResolvedValue({
                mensaje: "Monitoreo Motor creado con exito!",
                tipoAlerta: "success",
              }),
            actualizarMonitoreoMotor: null,
            obtenerMonitoreoMotor: jest.fn(),
          };
        ...
        await waitFor(() => {
            expect(mockMonitoreoMotorContext.registrarMonitoreoMotor).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Monitoreo Motor',
                activo: true,
            });
        });

        const mensajeExitoso = screen.getByText("Monitoreo Motor creado con exito!");
