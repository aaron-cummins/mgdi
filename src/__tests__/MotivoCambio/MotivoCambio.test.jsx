...
import FormMotivoCambio from 'pages/motivoCambio/components/FormMotivoCambio';
import MotivoCambio from 'pages/motivoCambio/MotivoCambio';
...
describe('Motivo Cambio', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Motivo Cambio')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Motivo Cambio" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Nuevo Motivo Cambio");
    }) 
    ...
    it('Verifica que se muestre el mensaje de error cuando los campos obligatorios se dejan en blanco al hacer clic en el botón guardar', () => {
        ...
        expect(screen.getByText(/Nombre motivo de cambio requerido/i)).toBeInTheDocument();
      });
    ...
    it('Verifica que el mensaje de error desaparezca cuando se ingresan datos válidos en el campo Nombre del formulario', () => {
        ...
        fireEvent.change(InputNombreElemento , { target: { value: 'Motivo cambio 1' } });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockMotivoCambioContext = {
            MotivoCambioActual: null,
            registrarMotivoCambio: jest.fn(),
            actualizarMotivoCambio: jest.fn(),
            obtenerMotivoCambio: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <MotivoCambioContext.Provider value={mockMotivoCambioContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormMotivoCambio closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Motivo Cambio 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockMotivoCambioContext = {
            MotivoCambioActual: null,
            registrarMotivoCambio: jest.fn().mockResolvedValue({
                mensaje: "Motivo Cambio creado con exito!",
                tipoAlerta: "success",
              }),
            actualizarMotivoCambio: null,
            obtenerMotivoCambio: jest.fn(),
          };
        ...
        await waitFor(() => {
            expect(mockMotivoCambioContext.registrarMotivoCambio).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Motivo Cambio',
                activo: true,
            });
        });
