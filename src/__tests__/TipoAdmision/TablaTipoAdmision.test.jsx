...
import TablaTipoAdmision from 'pages/tipoAdmision/components/TablaTipoAdmision';
import { ContextProvider } from 'contexts/ContextProvider';
import { TipoAdmisionContext } from 'pages/tipoAdmision/context/tipoadmisionContext';
import { ColActivoTabla, OpcionesTabla, Tabla } from "components";

describe('Tipo Admisión', () => {
    ...
    it('Verificar que las columnas "Id", "Nombre", "Activo" y "Acciones" se rendericen correctamente', () => {
        ...
        const mockTipoAdmisionList = [
            { id: 1, nombre: "Tipo Admision 1", activo: true },
            { id: 2, nombre: "Tipo Admision 2", activo: false },
          ];
        
        const obtenerTipoAdmision = jest.fn()
        const obtenerTipoAdmisiones = jest.fn()
        
        //Renderización de la tabla
        render(
            <ContextProvider>
                <TipoAdmisionContext.Provider value={{ tipoadmisionList: mockTipoAdmisionList, obtenerTipoAdmision, obtenerTipoAdmisiones}}>
                    <TablaTipoAdmision openModal={() => {}}/>
    ...
    it('Verifica que la columna “Activo” muestre los botones con un “SI” o “NO” según el valor de la propiedad “Activo” de cada fila. ', () => {
        ...
        const primerActivoTrue = mockTipoAdmisionList[0].activo
        const segundoActivoFalse = mockTipoAdmisionList[1].activo
   ...
   /**4. Renderización del formulario" */
    it('Verificar que al hacer clic en el icono editar se muestre el formulario', () =>{
        ...
        const obtenerTipoAdmisiones = jest.fn().mockResolvedValue();
        const obtenerTipoAdmision = jest.fn().mockResolvedValue();
        ...
        render(
            <ContextProvider>
                <TipoAdmisionContext.Provider value={{ tipoadmisionList: mockTipoAdmisionList, obtenerTipoAdmision, obtenerTipoAdmisiones}}>
                    <TablaTipoAdmision openModal={openModalMock}/>
                </TipoAdmisionContext.Provider>
            </ContextProvider>
        )
        ...
        expect(obtenerTipoAdmision).toHaveBeenCalled();
    ...
    it('Verificar que los datos obtenidos de la API se muestran correctamente en la tabla', async () => {
        const mockTipoAdmisionList = [
            { id: 1, nombre: "Tipo Admision 1", activo: true },
            { id: 2, nombre: "Tipo Admision 2", activo: false },
            { id: 3, nombre: "Tipo Admision 3", activo: true },
          ];
        
        const obtenerTipoAdmisiones = jest.fn().mockResolvedValue(mockTipoAdmisionList);
        const obtenerTipoAdmision = jest.fn().mockResolvedValue();
        const openModalMock = jest.fn();
        ...
        expect(rows.length).toBe(mockTipoAdmisionList.length +1);
    })
    ...
    it("Verificar que se muestre el mensaje 'No se encontraron datos para mostrar' cuando no se devuelvan datos en la API: ", async () => {
        const mockTipoAdmisionValue = {
            tipoadmisionList: [],
            obtenerTipoAdmision: jest.fn(),
            obtenerTipoAdmisiones: jest.fn(),
        };
        const { getByText } = render(
            <TipoAdmisionContext.Provider value={mockTipoAdmisionValue}>
                <TablaTipoAdmision />
            </TipoAdmisionContext.Provider>
        );
    ...
    it('Verificar que el componente OpcionesTabla se renderice correctamente', () => {
        render(<OpcionesTabla editar={true} nombreform="tipo admisión" />);
