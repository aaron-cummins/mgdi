...
import FormTipoAdmision from 'pages/tipoAdmision/components/FormTipoAdmision';
import TipoAdmision from 'pages/tipoAdmision/TipoAdmision';
...
describe('Tipo Admisión', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Tipo Admisión')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Tipo Admisión" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Nuevo Tipo Admisión");
    }) 
    ...
    it('Verifica que se muestre el mensaje de error cuando los campos obligatorios se dejan en blanco al hacer clic en el botón guardar', () => {
        ...
        expect(screen.getByText(/Nombre tipo admisión requerido/i)).toBeInTheDocument();
      });
    ...
    it('Verifica que el mensaje de error desaparezca cuando se ingresan datos válidos en el campo Nombre del formulario', () => {
        ...
        fireEvent.change(InputNombreElemento , { target: { value: 'Tipo Admisión 1' } });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockTipoAdmisionContext = {
            tipoadmisionActual: null,
            registrarTipoAdmision: jest.fn(),
            actualizarTipoAdmision: jest.fn(),
            obtenerTipoAdmision: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <TipoAdmisionContext.Provider value={mockTipoAdmisionContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormTipoAdmision closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Tipo Admision 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockTipoAdmisionContext = {
            tipoadmisionActual: null,
            registrarTipoAdmision: jest.fn().mockResolvedValue({
                mensaje: "Tipo Admisión creada con exito!",
                tipoAlerta: "success",
              }),
            actualizarTipoAdmision: null,
            obtenerTipoAdmision: jest.fn(),
          };
        ...
        await waitFor(() => {
            expect(mockTipoAdmisionContext.registrarTipoAdmision).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Tipo Admisión',
                activo: true,
            });
        });

        const mensajeExitoso = screen.getByText("Tipo Admisión creada con exito!");
