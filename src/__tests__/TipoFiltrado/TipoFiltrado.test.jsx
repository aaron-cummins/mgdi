...
import FormTipoFiltrado from 'pages/tipoFiltrado/components/FormTipoFiltrado';
import TipoFiltrado from 'pages/tipoFiltrado/TipoFiltrado';
...
describe('Tipo Filtrado', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Tipo Filtrado')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Tipo Filtrado" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Nuevo Tipo Filtrado");
    }) 
    ...
    it('Verifica que se muestre el mensaje de error cuando los campos obligatorios se dejan en blanco al hacer clic en el botón guardar', () => {
        ...
        expect(screen.getByText(/Nombre de tipo filtrado requerido/i)).toBeInTheDocument();
      });
    ...
    it('Verifica que el mensaje de error desaparezca cuando se ingresan datos válidos en el campo Nombre del formulario', () => {
        ...
        fireEvent.change(InputNombreElemento , { target: { value: 'Tipo filtrado 1' } });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockTipoFiltradoContext = {
            tipofiltradoActual: null,
            registrarTipoFiltrado: jest.fn(),
            actualizarTipoFiltrado: jest.fn(),
            obtenerTipoFiltrado: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <TipoFiltradoContext.Provider value={mockTipoFiltradoContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormTipoFiltrado closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Tipo Filtrado 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockTipoFiltradoContext = {
            tipofiltradoActual: null,
            registrarTipoFiltrado: jest.fn().mockResolvedValue({
                mensaje: "Tipo Filtrado creado con exito!",
                tipoAlerta: "success",
              }),
            actualizarTipoFiltrado: null,
            obtenerTipoFiltrado: jest.fn(),
          };
        ...
        await waitFor(() => {
            expect(mockTipoFiltradoContext.registrarTipoFiltrado).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Tipo Filtrado',
                activo: true,
            });
        });

        const mensajeExitoso = screen.getByText("Tipo Filtrado creado con exito!");
