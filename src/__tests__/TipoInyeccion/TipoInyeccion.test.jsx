...
import FormTipoInyeccion from 'pages/tipoInyeccion/components/FormTipoInyeccion';
import TipoInyeccion from 'pages/tipoInyeccion/TipoInyeccion';
...
describe('Tipo Inyección', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Tipo Inyección')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Tipo Inyección" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Nuevo Tipo Inyección");
    }) 
    ...
    it('Verifica que se muestre el mensaje de error cuando los campos obligatorios se dejan en blanco al hacer clic en el botón guardar', () => {
        ...
        expect(screen.getByText(/Nombre de tipo inyección requerido/i)).toBeInTheDocument();
      });
    ...
    it('Verifica que el mensaje de error desaparezca cuando se ingresan datos válidos en el campo Nombre del formulario', () => {
        ...
        fireEvent.change(InputNombreElemento , { target: { value: 'Tipo inyección 1' } });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockTipoInyeccionContext = {
            tipoinyeccionActual: null,
            registrarTipoInyeccion: jest.fn(),
            actualizarTipoInyeccion: jest.fn(),
            obtenerTipoInyeccion: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <TipoInyeccionContext.Provider value={mockTipoInyeccionContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormTipoInyeccion closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Tipo inyección 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockTipoInyeccionContext = {
            tipoinyeccionActual: null,
            registrarTipoInyeccion: jest.fn().mockResolvedValue({
                mensaje: "Tipo inyección creado con exito!",
                tipoAlerta: "success",
              }),
            actualizarTipoInyeccion: null,
            obtenerTipoInyeccion: jest.fn(),
          };
        ...
        await waitFor(() => {
            expect(mockTipoInyeccionContext.registrarTipoInyeccion).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Tipo Inyección',
                activo: true,
            });
        });

        const mensajeExitoso = screen.getByText("Tipo inyección creado con exito!");
