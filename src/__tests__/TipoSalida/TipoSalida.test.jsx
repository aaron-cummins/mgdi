...
import FormTipoSalida from 'pages/tipoSalida/components/FormTipoSalida';
import TipoSalida from 'pages/tipoSalida/TipoSalida';
...
describe('Tipo Salida', () => {
    ...
    it('Verificar la existencia del campo Nombre, el botón Guardar, el botón Cancelar, la casilla de verificación Activo y el icono de cierre de modal en el formulario', () =>{
        ...
        const ButtonNuevoElemento = screen.getByText('Nuevo Tipo Salida')
    ...
    it('Verificar que se actualice el estado cuando se ingrese texto en el campo nombre', () =>{
        ...
        fireEvent.change(InputElemento, { target: { value: "Nuevo Tipo Salida" } });

        //Verifica si el valor se actualizó correctamente 
        expect(InputElemento.value).toBe("Nuevo Tipo Salida");
    }) 
    ...
    it('Verifica que se muestre el mensaje de error cuando los campos obligatorios se dejan en blanco al hacer clic en el botón guardar', () => {
        ...
        expect(screen.getByText(/Nombre tipo de salida requerido/i)).toBeInTheDocument();
      });
    ...
    it('Verificar que los campos se limpien y vuelvan a su estado inicial cuando se hace clic en el botón “Cancelar”', () => {
        ...
        const mockTipoSalidaContext = {
            TipoSalidaActual: null,
            registrarTipoSalida: jest.fn(),
            actualizarTipoSalida: jest.fn(),
            obtenerTipoSalida: jest.fn(),
          };
        
        render(
            <ContextProvider>
                <TipoSalidaContext.Provider value={mockTipoSalidaContext}>
                    <SnackbarProvider maxSnack={2} >
                        <FormTipoSalida closeModal={() => {}}/>
                    </SnackbarProvider>
        ...
        //Ingresa un valor en el campo 
        fireEvent.change(InputElemento, { target: { value: "Tipo Salida 1" } });
    ...
    it('Verificar si se muestra una notificación de éxito después de enviar el formulario con datos válidos', async () =>{
        ...
        const mockTipoSalidaContext = {
            TipoSalidaActual: null,
            registrarTipoSalida: jest.fn().mockResolvedValue({
                mensaje: "Tipo salida creado con exito!",
                tipoAlerta: "success",
              }),
            actualizarTipoSalida: null,
            obtenerTipoSalida: jest.fn(),
          };
        ...
        await waitFor(() => {
            expect(mockTipoSalidaContext.registrarTipoSalida).toHaveBeenCalledWith({ 
                id: 0,
                nombre: 'Nuevo Tipo Salida',
                activo: true,
            });
        });

        const mensajeExitoso = screen.getByText("Tipo salida creado con exito!");
